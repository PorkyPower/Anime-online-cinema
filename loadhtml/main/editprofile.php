                ...
                <div id=block-info>
                    ...
                    <div id=foot-block-info>
                        <div class='colwr' id="foot-block-text">
                                {{ PROFILE.BORDERS_LIST }}
                                <div class='unfoot-block-text'>
                                    <div>Аватар</div><div style='display:flex;'><a id=upavatar>Загрузить</a>{{ PROFILE.DELAVATAR }}<input hidden name=avatar type=text value="{{ PROFILE.AVATAR }}"></div>
                                </div>
                                <div class='unfoot-block-text'>
                                    <div>Обложка</div><div style='display:flex;'><a id=upbanner>Загрузить</a>{{ PROFILE.DELBANNER }}<input hidden name=banner type=text value="{{ PROFILE.BANNER }}"></div>
                                </div>
                                <div class='unfoot-block-text'>
                                    <div>Ник</div><div><input placeholder=Ник name=nickname id=inprofile type=text value="{{ PROFILE.NAME }}"></div>
                                </div>

                                <div class='unfoot-block-text'>
                                    <div>Имя</div><div><input placeholder=Имя name=nameuser id=inprofile type=text value="{{ PROFILE.NAMEUSER }}"></div>
                                </div>
                                <div class='unfoot-block-text'>
                                    <div>Дата рождения</div><div><input id=inprofile name=birthday type=date value="{{ PROFILE.BIRTHDAY }}"></div>
                                </div>
                                <div class='unfoot-block-text'>
                                    <div>Город</div><div><input placeholder=Город name=city id=inprofile type=text value="{{ PROFILE.CITY }}"></div>
                                </div>
                                <div class='unfoot-block-text'>
                                    <div id=saveblockbtn>
                    ...
                    <div id=foot-block-info>
                        <div id=foot-block-text >
                            <div id=gridfoot-block-text >
                                ...
                                <div>
                                    <a  href="/editprofile" class=checkedlink id=settingslink >Основная информация</a>
                                </div>
                                <div>
...
{{ SCRIPT.PROFILE }}
        ...
        $(document).on('click', '#userborders_list_object', function  () {
            let stylesborder = {
                background: $(this).css('background')
            }
            $('#avatar').children('#subs').children('#ava-prof').children('#border_user').attr('style', $(this).attr("s")+'background:'+$(this).css('background')+';');
            $('input[name=border]').val($(this).attr('num'));
